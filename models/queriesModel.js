...
const queries = {
   ...
   */
  allPatientsQuery: `
    SELECT u.user_id,u.name,u.last_name,u.email,u.phone,u.avatar,u.register_date,roles.role
    FROM users AS u
   ...
   */
  patientByEmailQuery: `
    SELECT u.user_id,u.name,u.last_name,u.email,u.phone,u.password,u.avatar,u.register_date,roles.role
    FROM users AS u
    INNER JOIN roles ON u.role = roles.role
	WHERE u.email=$1`,
   ...
   */
  patientByIdQuery: `
    SELECT u.user_id,u.name,u.last_name,u.email,u.phone,u.avatar,roles.role
    FROM users AS u
    INNER JOIN roles ON u.role = roles.role
	WHERE u.user_id=$1`,
  ...
  /**
   * Sentencia que actualiza (nombre,apellido,phone,pass,avatar) info pacientes por user_id
   */
  updatePatientQuery: `
    UPDATE users
    SET name=$1, last_name=$2, phone=$3, password=$4, avatar=$5
    WHERE user_id=$6`,
